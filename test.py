...
def test(test_model, logger):
    ...
    test_path = "/export/pigfarm/pandiqi/code/tf-cpn-master/data/cow/test"
    ...
    for label in labels:
        ...
        aid += 1
        cowData = dict(category_id=1, imgpath=imgname, headRect=rect, bbox=bbox, image_id=aid,
                       segmentation=[[]])
        dets.append(cowData)
    # for i,img_name in enumerate(test_img_names):
    #     det={}
    #     det['imgpath'] = os.path.join(test_path,img_name)
    #     det['image_id'] = i
    #     det['category_id'] = 1
    #     dets.append(det)
