...
class COCOJoints(object):
    def __init__(self):
        ...
        for mpi, stage in zip([self.mpi, self.test_mpi], ['train','val']):
            if stage == 'train':
                self._train_gt_path=os.path.join(cur_dir, 'MSCOCO', 'annotations', 'person_keypoints_train2014.json')
                coco = COCO(self._train_gt_path)
            else:
                self._val_gt_path=os.path.join(cur_dir, 'MSCOCO', 'annotations', 'person_keypoints_val2014.json')
            ...
            if stage == 'train':
                for aid in coco.anns.keys():
                    ...
                    prefix = "/export/pigfarm/pandiqi/code/tf-cpn-master/data/COCO/MSCOCO/images/"
                    ...
                    imgname = prefix + str(ann['image_id']).zfill(12) + '.jpg'
                ...
                for img_id,img_info in files:
                    ...
                    imgname = prefix + str(img_id).zfill(12) + '.jpg'
